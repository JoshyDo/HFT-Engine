...
public:
    int parse(std::string_view raw, MarketTick& out, std::string_view& symbol_out) override {
        ...
        const auto* payload =
            reinterpret_cast<const phase7::SbeTickPayload*>(raw.data() + sizeof(phase7::SbeMessageHeader));
...
int main() {
    ...
    for (int i = 0; i < ITERATIONS; ++i) {
        mixed_parsers[i] =
            dist(rng) ? static_cast<IVirtualParser*>(&sbe_inst) : static_cast<IVirtualParser*>(&poison_inst);
    }
    ...
    std::cout << "[RDTSC] Starting Micro-Benchmark (Poisoned Branch Predictor)...\n";

    // --- TEST A: CRTP (Static, inlined, no VTable) ---
    for (int i = 0; i < ITERATIONS; ++i) {
        timer.start();
    ...
    auto print_stats = [](const std::string& name, std::vector<uint64_t>& cycles) {
        ...
        std::cout << name << " -> Min: " << min_cycles << " | Mean: " << mean << " | 99.9th: " << p999 << " Cycles\n";
    };
